...
<div class="dataTables_paginate paging_simple_numbers" id="pages-table_paginate">
    <ul class="pagination">
        @if($page>1)
        <li class="paginate_button page-item previous"><a href="{{ route('editora.action', 'list_instances/?p_pagina='.($page-1).'&p_class_id='.$class['id']) }}"  class="page-link">{{__('editora_lang::messages.paginacion_anteriores')}}</a></li>
        @endif
        @for($i=1; $i<=$pages; $i++)
        ...
        @endfor
        @if($page<$pages)
        <li class="paginate_button page-item next"><a href="{{ route('editora.action', 'list_instances/?p_pagina='.($page+1).'&p_class_id='.$class['id']) }}" class="page-link">{{__('editora_lang::messages.paginacion_siguientes')}}</a></li>
        @endif
    </ul>
</div>
@endif
