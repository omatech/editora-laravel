...
class InstancesModel extends Model {
    ...
    function instanceList($param_arr)
    {
        ...
        $p_search_query = isset($param_arr['search_query'])?$param_arr['search_query']:'';
        $p_class_id = isset($param_arr['class_id'])?$param_arr['class_id']:'';
        
        $fecha_ini = isset($param_arr['fecha_ini'])?$param_arr['fecha_ini']:'';
        $p_fecha_ini = str_replace("-", "/", $fecha_ini);
        $fecha_fin = isset($param_arr['fecha_fin'])?$param_arr['fecha_fin']:'';
        $p_fecha_fin = str_replace("-", "/", $fecha_fin);
        
        $p_pagina = isset($param_arr['pagina'])?$param_arr['pagina']:'';
        $p_order_by = isset($param_arr['order_by'])?$param_arr['order_by']:'';
        $p_search_state = isset($param_arr['search_state'])?$param_arr['search_state']:'';
        $p_mode = isset($param_arr['mode'])?$param_arr['mode']:'';
        $p_parent_inst_id = isset($param_arr['parent_inst_id'])?$param_arr['parent_inst_id']:'';
        $p_relation_id = isset($param_arr['relation_id'])?$param_arr['relation_id']:'';
    ...
    function getInstInfo($p_inst_id = null)
    {
        ...
        /*$ret['cache_option']=0;
        if ($this->hasCacheOption($p_inst_id)) $ret['cache_option']=1;
        $ret['cache_functions']=0;
        if ($this->hasCacheFunctions($p_inst_id)) $ret['cache_functions']=1;*/
 ...
 // Asumim ordre de insert TEXT,DATE,NUM, img_info
        $p_valor = str_replace("\xc2\xa0", ' ', $p_valor);
                    ...
                    if (is_array($valor)) {// L'atribut te varios valors, es un lookup de tipus checkbox
                        ...
                        switch ($type) {
                            case "Z":
                                ...
                                break;
                            case "W":
                                require_once($_SERVER['DOCUMENT_ROOT'].'/utils/appslib/Appscrapper.php');
