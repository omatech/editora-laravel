@if($p_mode=='R')
    ...
    <form class="form" id="relation_all" name="relation_all" method="post" enctype="multipart/form-data" action="{{route('editora.action', 'join_all')}}">
        {{ csrf_field() }}
        <button class="btn clr-secondary" style="font-size:14px">{{__('editora_lang::messages.info_word_join')}}</button>
...
<table class="table main-table" id="pages-table">
    ...
    <tr>
        ...
        <th class="picture"><span>{{__('editora_lang::messages.info_word_img')}}</span></th>
        ...
        <th class="creation-date"><span>{{__('editora_lang::messages.info_word_update_date')}}</span></th>
        <th class="date-condition"><span class="hidden">{{__('editora_lang::messages.info_word_date_condition')}}</span></th>
         ...
         @foreach($instances as $item)
            ...
            <tr>
                ...
                @if($p_mode=='R')
                    ...
                    <td class="actions">
                        {{-- <a href="{{$link}}" class="btn-square clr-mid"><i class="icon-link-rel"></i><span class="hide-txt">{{__('editora_lang::messages.info_word_join')}}</span> </a> --}}
                    </td>
                    @php($id_rel++)
                @else
                    <td class="actions">
...
@if($count!=-1)
<h3>{{$count}} {{__('editora_lang::messages.info_objects_found')}}</h3>
@endif
@if($count>40)
...
<div class="dataTables_paginate paging_simple_numbers" id="pages-table_paginate">
    <ul class="pagination">
        @if($page>1)
        <li class="paginate_button page-item previous"><a href="{{ route('editora.list_instances', ['pagina'=>$page-1, 'class_id' => $class['id']]) }}"  class="page-link">{{__('editora_lang::messages.paginacion_anteriores')}}</a></li>
        @endif
        @for($i=1; $i<=$pages; $i++)
        <li class="paginate_button page-item @if($i==$page) active @endif"><a href="{{ route('editora.list_instances', ['pagina'=>$i, 'class_id' => $class['id']]) }}" class="page-link">{{$i}}</a></li>
        @endfor
        @if($page<$pages)
        <li class="paginate_button page-item next"><a href="{{ route('editora.list_instances', ['pagina'=>$page+1, 'class_id' => $class['id']]) }}" class="page-link">{{__('editora_lang::messages.paginacion_siguientes')}}</a></li>
        @endif
    </ul>
</div>
@endif
