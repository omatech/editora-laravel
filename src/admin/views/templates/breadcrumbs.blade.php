    ...
    <ul>
        @if(isset($instance))
            @if(isset($instance['class_name']))
                <li><a href="{{route('editora.action', "get_main")}}">{{getMessage('navigation_home')}}</a></li>
            ...
            @endif
        @else
            <li class="active"><a href="{{route('editora.action', "get_main")}}">{{getMessage('navigation_home')}}</a></li>
        @endif
    </ul>
</nav>