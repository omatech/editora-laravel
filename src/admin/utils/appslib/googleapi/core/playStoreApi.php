	...
	class PlayStoreApi{
		...
		function topNewFreeApps($start = 1){
			...
			if(isset($this_content[0])){
				...
				$list_items = sgot('.'.$elements_to_look.' > li');
				$break_all_items = explode('</li>',$list_items);
				$break_all_items = str_replace('<li>','',$break_all_items);
				foreach($break_all_items as $li)
				{
		...
		function itemInfo($item_id){
			...
			phpQuery::newDocumentHTML($this_content[0]);
				...
				//if($app_price == 'Install' || 'Instalar') { $app_price = 'Free'; }
				$html_app_description = pq('#doc-original-text')->html();
				$text_plain_app_description = strip_tags($html_app_description);
                                $average_rating_value=pq('#details-tab-2 div div div div > div.average-rating-value')->html();     
                                $total_votes_value=pq('#details-tab-2 div div div div > div.votes')->html();       
                                $video=pq('.doc-video > embed')->attr('src');            
				foreach(pq('.screenshot-carousel-content-container > div > img') as $appshots){
					
                                        $app_screen_shots_ = pq($appshots)->attr('src');
                                        $app_screen_shots=preg_replace("/(=[a-z][0-9]+)$/i"," ", $app_screen_shots_);
				...
				$developer_website = pq(".doc-overview > a.more-arrow-link")->attr('href');
				...
				$app_info['General'][] = (object) array('video'=>$video,'total_votes_value'=>$total_votes_value,'average_rating_value'=>$average_rating_value,'app_store_url' => $app_store_url,'banner_image' => $banner_image,'banner_icon' => $banner_icon,'app_title' => $app_title, 'app_author' => $app_author,'author_store_url' => $author_store_url,
